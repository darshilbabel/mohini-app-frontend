...
import {
  languageList,
  sessionFlowName,
} from "../pages/ShikshalokamVoiceChat/enum";
...
function GuestPage() {
  ...
  const setFinalLanguage = async () => {
    ...
    if (
      currentFlow &&
      [sessionFlowName.GuestMiStory, sessionFlowName.GuestDiscussion].includes(
        currentFlow
      )
    ) {
      await initialSetup();
  ...
  async function initialSetup() {
    try {
      ...
      if (!!response?.access_token) {
        ...
        localStorage.setItem(
          "first_name",
          JSON.stringify(response?.first_name)
        );
      } else {
    ...
    <div className="container max-w-full md mt-0 mx-auto grid md:grid-cols-2 justify-center h-screen">
      ...
      <div className="px-5 hidden sm:block">
        ...
        <div className="flex flex-col items-center sm:mt-[150px]">
          <ol
            className="text-left sm:text-2xl text-md text-slate-700 list-decimal pl-5"
            style={{ color: "#6eafaf" }}
          >
            <li>{t("start_chat")}</li>
            <li>{t("add_photos")}</li>
