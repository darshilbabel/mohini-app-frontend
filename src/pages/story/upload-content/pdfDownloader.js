...
const PdfDownloader = ({
  ...
  useEffect(() => {
    ...
    let images;
    storyMediaIdArray
      ? (images = storyMediaIdArray.map((y) => y?.base64_str))
      : (images = []);
  ...
  async function autoExpandOnce() {
    ...
    textbox.map((subBox) => {
      return (
        ...
        (totalHeight += 20),
        (subBox.style.height =
          totalHeight <= parseInt(window.getComputedStyle(subBox).maxHeight)
            ? totalHeight + "px"
            : window.getComputedStyle(subBox).maxHeight)
      );
    });
  }
  ...
  function wordCounter(storyText) {
    storyText = JSON.parse(storyData?.formatted_content)?.filter(
      (x) => x.type === "paragraph"
    );
  ...
  async function uploadFile(formData, fileData, fileName, mediaId, storyId) {
    const story_media = storyData?.story_media.filter(
      (item) => item?.media_type === "application/pdf"
    );
  ...
  async function captureAndConvertToJPG() {
    ...
    return new Promise((resolve, reject) => {
      ...
      const promises = uniqueElements.map((element) =>
        captureAndConvertContainer(element)
      );
  ...
  function addPages() {
    if (
      !storyMediaIdArray ||
      !storyData ||
      !storyData?.formatted_content ||
      (contentPDF?.content1 === "" && contentPDF?.content2 === "") ||
      !current_company ||
      !currentState
    )
      return;
    ...
    if (images.length !== 0 && images.length % 2 === 0) {
      ...
      return (
        <>
          {currentState === "Nagaland" && <StoryFifthPage {...storyData} />}
          {contentPDF.content1[0] === "" &&
            pages.map((page, pageIndex) => (
              <StorySecondPage
                key={pageIndex}
                content1={contentPDF.content2}
                images={page}
                fontSize="1.1rem"
                tweet={contentPDF.tweetContent}
                imgPerPage={imagesPerPage}
                title={storyData?.title}
              />
            ))}
      ...
      if (index === contentPDF.content1.length - 1) {
        return (
          <StoryThirdPage
            ...
            fontSize="1.1rem"
            tweet={contentPDF.tweetContent}
            shouldShowStoryHeading={isHeadingVisible}
            title={storyData?.title}
          />
        );
      } else {
